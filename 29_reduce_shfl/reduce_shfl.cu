...
// Thread bundle statute summation and it runs on a warp-by-warp basis.
...
__global__ void reduceShfl(int *g_idata, int *g_odata, unsigned int n)
{
	...
	//Store the sum of each thread bundle
	if (laneIdx == 0)
		smem[warpIdx] = mySum;
	__syncthreads();
	//The dim.x of the block is 1024, and the warps has 32
	//The previous sem[0... .31] stores the sum of the 32 warps
	//mysum<-sem[0...31]

	mySum = (threadIdx.x < DIM) ? smem[laneIdx] : 0;
	//The first warp is summed over the entire block, 
	//using the sum of the 32 previously computed warps
	if (warpIdx == 0)
		mySum = warpReduce(mySum);
		
	//Returns the sum of the blockIdx.x blocks
	if (threadIdx.x == 0)
