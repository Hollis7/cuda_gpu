...
__global__ void reduceUnrollWarp8(int *g_idata, int *g_odata, unsigned int n)
{
	...
	// write result for this block to global mem
	// consistent progress of execution within the same Thread bundle
	/*volatile:Tell the compiler not to optimize the variable at compile time
	and to still read the variable on reread*/
	if (tid < 32)
	{
